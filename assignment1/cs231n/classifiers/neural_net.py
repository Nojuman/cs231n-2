...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #############################################################################
    temp = np.sum(np.exp(l3), axis=1)
    l4 = np.sum(np.log(temp) - l3[np.arange(N), y]) / N
    loss = l4 + 0.5 * reg * (np.sum(W1 * W1) + np.sum(W2 * W2))
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    dl3 = (np.exp(l3).T / temp).T
    ys = np.zeros_like(dl3)
    ys[np.arange(N), y] = 1
    dl3 = (dl3 - ys) / N * dl4
    ...
    dW1 = X.T.dot(dl1) + reg * W1
    dW2 = l2.T.dot(dl3) + reg * W2
