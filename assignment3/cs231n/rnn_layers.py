...
def rnn_forward(x, h0, Wx, Wh, b):
  ...
  cache = []
  for i in xrange(T):
    cur_h, cur_cache = rnn_step_forward(x[:, i, :], cur_h, Wx, Wh, b)
    h[:, i, :] = cur_h
...
def rnn_backward(dh, cache):
  ...
  for i in xrange(T - 1, -1, -1):
    dx[:, i, :], cur_dh, cur_dWx, cur_dWh, cur_db = rnn_step_backward(dh[:, i, :] + cur_dh, cache[i])
    dWx += cur_dWx
    dWh += cur_dWh
    db += cur_db
  dh0 = cur_dh
...
def lstm_step_backward(dnext_h, dnext_c, cache):
  ...
  da3 = do * o * (1 - o)
  da4 = dg * (1 - g**2)
  dactivation = np.hstack((da1, da2, da3, da4))
...
def lstm_forward(x, h0, Wx, Wh, b):
  ...
  cache = []
  for i in xrange(T):
    cur_h, cur_c, cur_cache = lstm_step_forward(x[:, i, :], cur_h, cur_c, Wx, Wh, b)
    h[:, i, :] = cur_h
    cache.append(cur_cache)
  ##############################################################################
  #                               END OF YOUR CODE                             #
...
def lstm_backward(dh, cache):
  ...
  cur_dc = np.zeros((N, H))
  for i in xrange(T - 1, -1, -1):
    dx[:, i, :], cur_dh, cur_dc, cur_dWx, cur_dWh, cur_db = lstm_step_backward(dh[:, i, :] + cur_dh, cur_dc, cache[i])
    dWx += cur_dWx
    dWh += cur_dWh
    db += cur_db
  dh0 = cur_dh
